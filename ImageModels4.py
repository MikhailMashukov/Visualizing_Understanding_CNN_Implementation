...
#             kwargs['kernel_initializer'] = 'he_normal'
        convBlock = Conv2D(filters * sizeMult, kernel_size,
                           activity_regularizer=keras.regularizers.l2(1e-7), **kwargs)
        # print('he_normal')
...
def DebugImageModel4(classCount=DeepOptions.classCount, doubleSizeLayerNames=[]):
    import tensorflow as tf

    inputs = Input(shape=(227, 227, 3))
    mult = DeepOptions.netSizeMult

    (conv_1, chans) = ModelBlock4(mult * 8, 7, strides=2, activation='relu',
                    name='conv_1', doubleSizeLayerNames=doubleSizeLayerNames)(inputs, None)
    conv_2 = MaxPooling2D(3, strides=3)(conv_1)

    (conv_2, chans) = ModelBlock4(mult * 16, 5, strides=2, activation='relu',
                    name='conv_2', doubleSizeLayerNames=doubleSizeLayerNames)(conv_2, chans)
    conv_3 = MaxPooling2D(2, strides=2)(conv_2)

    conv_3 = BatchNormalization()(conv_3)
    (conv_3, chans) = ModelBlock4(mult * 16, 5, strides=2, activation='relu',
                    name='conv_3', doubleSizeLayerNames=doubleSizeLayerNames)(conv_3, chans)
    dense_1 = MaxPooling2D(2, strides=2)(conv_3)

    dense_regularizer = keras.regularizers.l2(1e-7)
    dense_1 = Flatten(name="flatten")(dense_1)
    size = (mult * classCount // 2) if classCount > 100 else (mult * 16)
    dense_1 = Dense(size, activation='relu', # activity_regularizer=keras.regularizers.l1(1e-6),
                    kernel_initializer='he_normal', activity_regularizer=dense_regularizer,
                    name='dense_1')(dense_1)
    ...
    dense_2 = Dropout(0.4)(dense_2)
    size = (mult * classCount // 2) if classCount > 100 else (mult * 8)
    dense_2 = Dense(size, kernel_initializer='he_normal', activity_regularizer=dense_regularizer,
                    name='dense_2')(dense_2)

    dense_3 = BatchNormalization()(dense_2)
    # dense_3 = Dropout(0.3)(dense_2)
    dense_3 = Dense(classCount, kernel_initializer='he_normal', activity_regularizer=dense_regularizer,
                    name='dense_3')(dense_3)
    prediction = Activation("softmax", name="softmax")(pow(1.2)(dense_3))
